export interface TrebleClefProps {
  className?: string
  // Accepted for forward compatibility with the optional Unicode-glyph render
  // path documented in research.md R5; v1 always renders the SVG.
  forceSvg?: boolean
}

// Hollow treble clef. Source: rickvanderzwet on Openclipart
// (https://openclipart.org/detail/12473), released to the public domain.
// Adapted here as a React component: the original stroke + default fill are
// replaced with `fill="currentColor"` so the clef adopts the theme color.
export function TrebleClef({ className }: TrebleClefProps) {
  return (
    <svg
      role="img"
